...
```
if (-Not (Test-Path C:\temp)) { mkdir C:\temp }; mkdir C:\temp\es-setup\; Set-Location C:\temp\es-setup\;Set-ExecutionPolicy Bypass -Scope Process -Force;[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12;iwr -useb get.scoop.sh | iex
;Invoke-WebRequest "https://github.com/Francommit/win10_emulation_station/archive/1.1.1.zip" -OutFile "emulationStation.zip";Expand-Archive .\emulationStation.zip;cd .\emulationStation; cd .\win*;.\prepare.ps1
```
